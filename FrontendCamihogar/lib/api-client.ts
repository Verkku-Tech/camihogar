...
const SECURITY_API_URL = process.env.NEXT_PUBLIC_SECURITY_API_URL || 'https://localhost:7144'
const USERS_API_URL = process.env.NEXT_PUBLIC_USERS_API_URL || 'https://localhost:7119'
...
export class ApiClient {
  private getBaseUrl(endpoint: string): string {
    ...
    if (endpoint.startsWith('/api/users') || endpoint.startsWith('/api/Users')) {
  ...
  async refreshToken(): Promise<boolean> {
    ...
    try {
      ...
      localStorage.setItem('token_expires_at', new Date(data.expiresAt).getTime().toString())
