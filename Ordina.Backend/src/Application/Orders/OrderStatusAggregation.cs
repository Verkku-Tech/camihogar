...
public static class OrderStatusAggregation
{
    public static string CalculateFromProducts(IEnumerable<OrderProduct> products)
    {
        ...
        {
            var status = product.LogisticStatusString ?? "Generado";
            if (status == "Completado")
                anyCompletado = true;

            var inFabricacion = IsFabricationLocation(product.LocationStatusString);
            var manufacturing = NormalizeManufacturingStatus(product.ManufacturingStatusString);
            ...
            else if (status == "Validado" || status == "Por Fabricar" || (inFabricacion && manufacturing == "debe_fabricar"))
                hasValidado = true;
        ...
        IReadOnlyList<OrderProduct> products)
    {
        ...
        var fabrication = products
            .Where(p => IsFabricationLocation(p.LocationStatusString))
            .ToList();
