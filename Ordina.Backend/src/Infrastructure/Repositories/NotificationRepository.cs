...
public class NotificationRepository : MongoRepository<Notification>, INotificationRepository
{
    ...
    private static FilterDefinition<Notification> BuildAudienceFilter(string userId, IEnumerable<string> roles)
    {
        ...
        var audience = fb.Or(userSpecificFilter, broadcastFilter, roleMatchFilter);
        var notDeleted = fb.Not(fb.AnyEq(n => n.DeletedByUserIds, userId));

        return audience & notDeleted;
    }

    public async Task<IReadOnlyList<Notification>> GetForUserAsync(string userId, IEnumerable<string> roles, int skip = 0, int limit = 10, CancellationToken ct = default)
    {
    ...
    public async Task<bool> DeleteAsync(string notificationId, string userId, CancellationToken ct = default)
    {
        ...
        var filter = fb.Eq(n => n.Id, notificationId);
        var update = ub.AddToSet(n => n.DeletedByUserIds, userId);
    ...
    public async Task<bool> DeleteAllAsync(string userId, IEnumerable<string> roles, CancellationToken ct = default)
    {
        ...
        var result = await _collection.UpdateManyAsync(audienceFilter, update, cancellationToken: ct);
        return result.ModifiedCount > 0;
    }
