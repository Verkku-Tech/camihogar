  ...
  const getAttributeValueLabel = (
    ...
    if (categoryAttribute.valueType === "Product") {
      const processProduct = (productId: any) => {
        ...
        if (
          subAttrs &&
          typeof subAttrs === "object" &&
          !Array.isArray(subAttrs)
        ) {
          const subCategory = categories.find(
            (c) => c.name === foundProduct.category,
          );
          if (subCategory && subCategory.attributes) {
            const subLabels = Object.entries(subAttrs)
              .map(([subKey, subVal]) => {
                const subAttrDef = subCategory.attributes.find(
                  (a: any) => a.id?.toString() === subKey || a.title === subKey,
                );
                if (subAttrDef) {
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div className="w-full sm:w-56">
                      ...
                      <Select
                        value={product.locationStatus ?? "SELECCIONAR ESTADO"}
                        onValueChange={(
                          value:
