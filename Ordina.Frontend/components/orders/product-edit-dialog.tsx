...
const getValueString = (value: string | AttributeValue): string => {
  if (typeof value === "string") {
    ...
    return trimmed !== "" ? trimmed : "unknown"; // Si es cadena vacía o solo espacios, usar "unknown"
  }
  const label = value.label?.trim() || "";
  const id = value.id?.trim() || "";
  return label !== "" ? label : id !== "" ? id : "unknown"; // Si ambos son undefined/vacíos/solo espacios, usar "unknown"
};

const getValueLabel = (value: string | AttributeValue): string => {
  if (typeof value === "string") {
    const trimmed = value.trim();
    return trimmed !== "" ? trimmed : "Sin nombre"; // Si es cadena vacía o solo espacios, usar "Sin nombre"
  }
  const label = value.label?.trim() || "";
  const id = value.id?.trim() || "";
  return label !== "" ? label : id !== "" ? id : "Sin nombre"; // Si ambos son undefined/vacíos/solo espacios, usar "Sin nombre"
};
      ...
      const processProductPrice = (productId: any) => {
        ...
        if (foundProduct) {
          ...
          if (pOriginalCurrency !== "Bs" && exchangeRates) {
            if (pOriginalCurrency === "USD" && exchangeRates.USD?.rate)
              pPriceInBs = pPrice * exchangeRates.USD.rate;
            else if (pOriginalCurrency === "EUR" && exchangeRates.EUR?.rate)
              pPriceInBs = pPrice * exchangeRates.EUR.rate;
          }
          ...
          if (subAttrs) {
            ...
            if (subCategory) {
              const subAdjustments = calculateDetailedAttributeAdjustments(
                subAttrs,
                subCategory,
                exchangeRates,
                allProducts,
              ...
              subAdjustments.forEach((subAdj) => {
                adjustments.push({
                  ...
                  adjustmentInOriginalCurrency:
                    subAdj.adjustmentInOriginalCurrency,
...
function ProductAttributesEditor({
  ...
  const [productPriceFormatted, setProductPriceFormatted] =
    useState<string>("");
  const [formattedAttributeAdjustments, setFormattedAttributeAdjustments] =
  ...
  useEffect(() => {
    const calculateAndFormatAdjustments = async () => {
      ...
      for (const attribute of category.attributes) {
        ...
        const attrValue =
          attributes[attrKey] ??
          (attribute.id ? attributes[attribute.id.toString()] : undefined);
        ...
        if (Array.isArray(attrValue)) {
          totalAdjustment = attrValue.reduce((total, valStr) => {
            ...
            return (
              total + (selectedValue ? getPriceAdjustment(selectedValue) : 0)
            );
          }, 0);
    ...
    switch (attribute.valueType) {
      case "Number":
        const numValue = attrValue
          ? parseFloat(attrValue.toString())
          : undefined;
        const minValue =
          attribute.minValue !== undefined ? attribute.minValue : undefined;
        const maxValue =
          attribute.maxValue !== undefined ? attribute.maxValue : undefined;
      ...
      case "Select":
        ...
        const selectedAdjustment = selectedValue
          ? getPriceAdjustment(selectedValue)
          : 0;
      ...
      case "Multiple select":
        ...
        // Función helper para convertir ajuste a Bs
        const convertAdjustmentToBsMultiple = (
          adjustment: number,
          currency?: string,
        ): number => {
          if (!currency || currency === "Bs") return adjustment;
        ...
        // Función helper para obtener el ajuste de precio de un valor
        const getPriceAdjustmentMultiple = (
        ...
        const totalAdjustment = selectedValues.reduce((total, valStr) => {
          ...
          return (
            total +
            (selectedValue ? getPriceAdjustmentMultiple(selectedValue) : 0)
          );
        }, 0);
            ...
            <div className="flex items-start gap-2">
              ...
              {totalAdjustment !== 0 && (
                ...
                >
                  {totalAdjustment > 0 ? "+" : ""}
                  {formattedAttributeAdjustments[attrKey] ||
                    formatCurrency(totalAdjustment, "Bs")}
                </span>
              )}
            </div>
          </div>
              ...
              <div key={attrKey} className="space-y-1.5">
                ...
                {(() => {
                  ...
                  const attrValue =
                    attributes[attrKey] ??
                    (attribute.id
                      ? attributes[attribute.id.toString()]
                      : undefined);
                  const isEmpty =
...
export function ProductEditDialog({
  ...
  const [calculatedBasePriceInBs, setCalculatedBasePriceInBs] =
    useState<number>(0);
  const [
    calculatedBasePriceWithAdjustments,
    setCalculatedBasePriceWithAdjustments,
  ] = useState<number>(0);
  const [
    calculatedProductAttributesTotal,
    setCalculatedProductAttributesTotal,
  ] = useState<number>(0);
  ...
  const [detailedAttributeAdjustments, setDetailedAttributeAdjustments] =
    useState<
      Array<{
        attributeName: string;
        selectedValueLabel: string;
    ...
    >([]);
  const [
    detailedProductAttributeAdjustments,
          ...
          if (existing && Array.isArray(existing) && existing.length > 0) {
            const updatedEntries = existing.map((entry) => {
              ...
              const attributesChanged =
                ...
                JSON.stringify(editedAttributes);
  ...
  useEffect(() => {
    const formatVisibleAdjustments = async () => {
      ...
      for (const attribute of currentCategory.attributes) {
        ...
        if (Array.isArray(attrValue)) {
          ...
          attrValue.forEach((valStr) => {
            ...
            if (
              ...
              "priceAdjustment" in selectedValue
            ) {
        ...
        if (totalAdjustment !== 0) {
          formatted[attrKey] = await formatWithPreference(
            totalAdjustment,
            "Bs",
          );
        }
    ...
    const updateFormattedTotals = async () => {
      ...
      const formattedAttributeAdjustments = await Promise.all(
        attributeAdjustments.map(async (adj) => ({
          ...
          formattedAdjustment: await formatWithPreference(
            adj.adjustmentInOriginalCurrency,
            adj.originalCurrency as Currency,
          ),
        })),
      );
        ...
        if (attribute.valueType === "Product") {
          ...
          for (const productEntry of productsForAttr) {
            ...
            const productCategory = categories.find(
              (cat) => cat.name === productEntry.product.category,
            );
            ...
            if (productCategory) {
              ...
              hasAttributeAdjustments =
                ...
                rawAdjustments.some((adj) => adj.adjustment !== 0);
            ...
            const formattedProductPrice = await formatWithPreference(
              productPrice,
              productCurrency,
            );
  ...
  const checkProductAttributeMissingMandatoryAttributes = (productEntry: {
    product: Product;
    attributes?: Record<string, any>;
  }): string[] => {
    const missingAttributes: string[] = [];
    ...
    if (
      ...
      productCategory.attributes.length > 0
    ) {
      ...
      for (const productAttr of productCategory.attributes) {
        ...
        if (
          productAttr.required !== false &&
          (productAttr.valueType === "Select" ||
            productAttr.valueType === "Multiple select")
        ) {
          const productAttrValue =
            editedAttributes[productAttrKey] ??
            (productAttr.title
              ? editedAttributes[productAttr.title]
              : undefined);
          ...
          if (productAttr.valueType === "Select") {
            ...
            if (
              !Array.isArray(productAttrValue) ||
              productAttrValue.length === 0
            ) {
              missingAttributes.push(productAttr.title || productAttrKey);
            }
          }
  ...
  const handleSave = () => {
    ...
    if (
      ...
      currentCategory.attributes.length > 0
    ) {
      const missingAttributes: string[] = [];
          ...
          if (
            ...
            if (isNaN(numValue)) {
              ...
              if (
                ...
                numValue < attribute.minValue
              ) {
                missingAttributes.push(
                  `${attribute.title || attrKey} (mínimo: ${attribute.minValue})`,
                );
              }
              // Validar maxValue si existe
              if (
                attribute.maxValue !== undefined &&
                numValue > attribute.maxValue
              ) {
                missingAttributes.push(
                  `${attribute.title || attrKey} (máximo: ${attribute.maxValue})`,
                );
              }
            }
          }
        ...
        if (attribute.valueType === "Product") {
          ...
          if (productsForAttribute.length === 0) {
            ...
            for (const productEntry of productsForAttribute) {
              const missingProductAttributes =
                checkProductAttributeMissingMandatoryAttributes(productEntry);
              if (missingProductAttributes.length > 0) {
                const missingList = missingProductAttributes.join(", ");
                missingAttributes.push(
                  `${attribute.title || attrKey} → ${productEntry.product.name}: ${missingList}`,
                );
              }
            }
          }
    ...
    if (currentCategory) {
      for (const attribute of currentCategory.attributes || []) {
        ...
        const value =
          attributes[attrTitle] ?? (attrId ? attributes[attrId] : undefined);
        ...
        if (attribute.valueType === "Product") {
          const productsForAttr =
            productAttributes[attrTitle] ??
            (attrId ? productAttributes[attrId] : []);

          // IMPORTANTE: Guardar el array de IDs de productos seleccionados usando título como clave
          if (productsForAttr.length > 0) {
            finalAttributes[attrTitle] = productsForAttr.map(
          ...
          for (const productEntry of productsForAttr) {
            ...
            if (
              productEntry.attributes &&
              Object.keys(productEntry.attributes).length > 0
            ) {
    ...
    const updatedProduct: OrderProduct = {
      ...
      surchargeAmount:
        surchargeEnabled && surchargeAmount > 0 ? surchargeAmount : undefined,
      surchargeReason:
        surchargeEnabled && surchargeReason.trim()
          ? surchargeReason.trim()
          : undefined,
    };
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        >
          ...
          <div className="space-y-3">
                          ...
                          ((attr.valueType === "Select" &&
                            ...
                            (attr.valueType === "Product" &&
                              (!productAttributes[attrKey] ||
                                productAttributes[attrKey].length === 0)));
                      ...
                      {attr.valueType === "Select" ? (
                              ...
                              {productsForAttribute.length > 0 ? (
                                <div className="grid grid-cols-1 gap-2">
                                  {productsForAttribute.map((productEntry) => {
                                    const missingProductAttributes =
                                      checkProductAttributeMissingMandatoryAttributes(
                                        productEntry,
                                      );
                                    const hasMissingAttributes =
                                      missingProductAttributes.length > 0;
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            if (
                              attr.minValue !== undefined &&
                              parsedValue < attr.minValue
                            ) {
                              toast.error(
                                `El valor debe ser mayor o igual a ${attr.minValue}`,
                              );
                              return;
                            }
                            if (
                              attr.maxValue !== undefined &&
                              parsedValue > attr.maxValue
                            ) {
                              toast.error(
                                `El valor debe ser menor o igual a ${attr.maxValue}`,
                              );
                              return;
                            }
            ...
            <div className="space-y-2">
              ...
              {surchargeEnabled && (
                <div className="space-y-2 pl-6 border-l-2 border-orange-500/30">
                  ...
                  <div className="space-y-1">
                    <Label
                      htmlFor="surchargeReason"
                      className="text-xs text-muted-foreground"
                    >
                      Razón del sobreprecio{" "}
                      <span className="text-red-500">*</span>
                    </Label>
                    <Input
                      id="surchargeReason"
            ...
            <div className="space-y-2">
              <Label htmlFor="productObservations" className="text-sm">
                Observaciones
              </Label>
              <Textarea
            ...
            <div className="p-3 bg-muted rounded-lg space-y-1.5">
              {(() => {
                ...
                const basePriceWithAdjustments =
                  calculatedBasePriceWithAdjustments || product.price;
                const productAttributesTotal =
                  calculatedProductAttributesTotal || 0;
                  ...
                  <>
                    ...
                    <div className="text-xs text-muted-foreground space-y-1">
                        ...
                        <span>
                          {basePriceFormatted ||
                            (originalProduct
                              ? formatCurrency(
                                  originalProduct.price,
                                  originalProduct.priceCurrency || "Bs",
                                )
                              : formatCurrency(product.price, "Bs"))}
                        </span>
                      </div>
                      {/* Mostrar cada atributo individualmente con su ajuste y valor seleccionado */}
                      {/* Solo mostrar atributos que tengan precio diferente de cero */}
                      {detailedAttributeAdjustments.filter(
                        (adj) => adj.adjustment !== 0,
                      ).length > 0 && (
                        <>
                          {detailedAttributeAdjustments
                            .filter((adj) => adj.adjustment !== 0)
                            .map((adj, idx) => (
                              <div key={idx} className="flex justify-between">
                                <span>
                                  {adj.attributeName}
                                  {adj.selectedValueLabel &&
                                    ` (${adj.selectedValueLabel})`}
                                  :
                                </span>
                                <span
                      ...
                      {detailedProductAttributeAdjustments.length > 0 && (
                        <>
                          {detailedProductAttributeAdjustments.map(
                            (productAttr, idx) => (
                              <div key={idx} className="space-y-1">
                                ...
                                {productAttr.productAdjustments.length > 0 && (
                                  <>
                                    {productAttr.productAdjustments.map(
                                      (adj, adjIdx) => (
                                        <div
                                          key={adjIdx}
                      ...
                      {surchargeEnabled && surchargeAmount > 0 && (
                        <div className="flex justify-between text-xs pt-1 text-orange-600 dark:text-orange-400 font-medium">
                          <span>
                            Sobre precio ({surchargeReason || "Sin razón"}):
                          </span>
