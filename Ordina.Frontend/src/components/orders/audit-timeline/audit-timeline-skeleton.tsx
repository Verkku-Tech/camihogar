...
export function AuditTimelineSkeleton({
  ...
  return (
    <div
      className="relative pl-8 space-y-4 before:absolute before:left-[11px] before:top-3 before:bottom-3 before:w-0.5 before:bg-border/30"
    ...
    >
      {Array.from({ length: count }).map((_, index) => (
        <div key={`skel-${index}`} className="relative">
          ...
          <div className="absolute -left-8 top-3.5 w-6 h-6 rounded-full bg-muted/60 border-2 border-border/40 animate-pulse" />
